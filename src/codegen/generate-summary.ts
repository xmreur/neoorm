import type { DestructiveChange, Manifest, ManifestDiff } from "../dialect/types.js";
import {
...
export function summarizeGenerateOutcome(params: {
  ...
  const { prev, next, diff, sql, blocked, schemaChanged, migrationName } = params;
  ...
  if (!schemaChanged) {
    return {
      ...
      migrationName: null,
      sqlStatementCount: 0,
      reasons: ["Snapshot matches schema — no manifest or migration changes."],
      blocked: [],
    };
  }
    ...
    if (manualBlocked.length > 0) {
      reasons.push(
        ...formatDestructiveWarnings(manualBlocked).map(
          (message) => `Skipped (manual migration required): ${message}`,
        ),
      );
    }
    return {
...
export function formatGenerateSummary(
  ...
  switch (summary.status) {
    ...
    case "codegen_only":
      ...
      lines.push(`  Updated ${outDir}/manifest.ts, ${outDir}/snapshot.json`);
      break;
    ...
    case "migration_blocked":
      lines.push("✗ No migration written — blocked changes require attention.");
      for (const reason of summary.reasons) {
