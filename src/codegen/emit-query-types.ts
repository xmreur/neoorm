...
function emitRepositoryType(table: ManifestTable): string {
	...
	return [
		...
		`  findMany<const T extends FindMany${model}Args = Record<never, never>>(args?: Exact<T, FindMany${model}Args>): Promise<${model}QueryResult<T>[]>;`,
		`  findFirst<const T extends FindMany${model}Args = Record<never, never>>(args?: Exact<T, FindMany${model}Args>): Promise<${model}QueryResult<T> | null>;`,
		`  findUnique<const T extends FindUnique${model}Args>(args: Exact<T, FindUnique${model}Args>): Promise<${model}QueryResult<T> | null>;`,
		`  findById<const T extends FindById${model}Args = Record<never, never>>(id: string | Record<string, unknown>, args?: Exact<T, FindById${model}Args>): Promise<${model}QueryResult<T> | null>;`,
		`  create<const T extends Create${model}Args>(args: Exact<T, Create${model}Args>): Promise<${model}CreateQueryResult<T>>;`,
		`  createMany(args: CreateMany${model}Args): Promise<number>;`,
		`  createManyAndReturn(args: CreateMany${model}Args): Promise<${model}[]>;`,
		`  upsert<const T extends Upsert${model}Args>(args: Exact<T, Upsert${model}Args>): Promise<${model}UpsertQueryResult<T>>;`,
		`  findOrCreate<const T extends FindOrCreate${model}Args>(args: Exact<T, FindOrCreate${model}Args>): Promise<{ record: ${model}QueryResult<T>; created: boolean }>;`,
		`  update<const T extends Update${model}Args>(args: Exact<T, Update${model}Args>): Promise<${model}UpdateQueryResult<T>>;`,
		`  updateMany(args: UpdateMany${model}Args): Promise<number>;`,
		`  updateManyAndReturn(args: UpdateMany${model}Args): Promise<${model}[]>;`,
		`  updateById<const T extends UpdateById${model}Args>(id: string | Record<string, unknown>, args: Exact<T, UpdateById${model}Args>): Promise<${model}UpdateQueryResult<T>>;`,
		`  delete<const T extends Delete${model}Args>(args: Exact<T, Delete${model}Args>): Promise<${model}DeleteQueryResult<T>>;`,
		...
		`  count<const T extends Count${model}Args = Record<never, never>>(args?: Exact<T, Count${model}Args>): Promise<Count${model}Result<T>>;`,
		...
		`  paginate<const T extends Paginate${model}Args>(args: Exact<T, Paginate${model}Args>): Promise<PaginateResult<${model}QueryResult<T>, Partial<${model}> | null>>;`,
		`}`,
	].join("\n");
}
