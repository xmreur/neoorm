...
async function removeGeneratedZod(outDir: string): Promise<void> {
	try {
		await unlink(join(outDir, "zod.ts"));
	} catch (err) {
...
export async function writeGeneratedFiles(
	...
	prev: Manifest | null,
	dialect?: Dialect,
	options?: { updateSnapshot?: boolean; zod?: boolean },
	...
	if (emitZod) {
		...
		await removeGeneratedZod(outDir);
	}
