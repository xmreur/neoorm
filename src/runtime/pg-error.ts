...
function findTableBySqlName(manifest: Manifest, sqlName: string | undefined): ManifestTable | undefined {
  if (!sqlName) return undefined;
  return Object.values(manifest.tables).find((table) => table.sqlName === sqlName);
}

function findTableByAccessor(manifest: Manifest, accessor: string | undefined): ManifestTable | undefined {
...
export function enrichPgError(
  err: PgErrorLike,
  manifest: Manifest,
  base: Pick<QueryErrorContext, "operation" | "sql"> & { tableAccessor?: string },
): QueryErrorContext {
  const table =
    findTableByAccessor(manifest, base.tableAccessor) ??
    findTableBySqlName(manifest, err.table);

  const { columnTsName, columnSqlName } = resolveColumnNames(table, err.column);

  const context: QueryErrorContext = {
    operation: base.operation,
    sql: truncateSql(base.sql),
    detail: err.detail ? `${headlineForPgCode(err)} (${err.detail})` : headlineForPgCode(err),
  };
