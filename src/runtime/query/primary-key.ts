...
import type { ManifestColumn, ManifestRelation, ManifestTable } from "../../dialect/types.js";
...
export function primaryKeySqlName(table: ManifestTable, index = 0): string {
  ...
  if (!sqlName) {
    throw new Error(`Primary key index ${index} not found for table "${table.accessor}"`);
  }
  return sqlName;
}

export function requireScalarPrimaryKey(table: ManifestTable): { tsName: string; sqlName: string } {
...
export function rowScalarPkValue(row: Record<string, unknown>, table: ManifestTable): string {
...
export function rowPkKey(row: Record<string, unknown>, table: ManifestTable): string {
