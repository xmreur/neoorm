...
export function resolveOrderSpec(
  ...
  for (const [tsKey, direction] of Object.entries(orderBy)) {
    ...
    if (!col) {
      throw new Error(`Unknown orderBy column "${tsKey}" on table "${table.accessor}"`);
    }
    const dir = direction.toLowerCase() === "desc" ? "desc" : "asc";
    directions.add(dir);
    specs.push({ tsName: tsKey, sqlName: col.sqlName, direction: dir, column: col });
  }

  if (directions.size > 1) {
    throw new Error("paginate orderBy columns must share the same direction");
  }

  const { tsName: pkTsName, sqlName: pkSqlName } = requireScalarPrimaryKey(table);
  if (!specs.some((spec) => spec.tsName === pkTsName)) {
    const pkCol = table.columns.find((c) => c.tsName === pkTsName);
    if (!pkCol) {
      throw new Error(`Primary key column not found for table "${table.accessor}"`);
    }
...
export function compileOrderByFromSpec(orderSpec: OrderKeySpec[]): string {
  const parts = orderSpec.map(
    (key) => `${quoteIdentifier(key.sqlName)} ${key.direction.toUpperCase()}`,
  );
...
export function compileCursorWhere(
  ...
  const colRefs = orderSpec.map((key) => quoteIdentifier(key.sqlName)).join(", ");
  const placeholders = orderSpec
    .map((_, index) => `$${startParamIndex + index}`)
    .join(", ");
  const params = orderSpec.map((key) => serializeColumnValue(key.column, cursor[key.tsName]));
