...
export async function paginateRecords(
  ...
  if (args.after) {
    const cursorWhere = compileCursorWhere(orderSpec, args.after, userParams.length + 1);
    const merged = mergeWhereWithCursor(userWhereSql, userParams, cursorWhere);
    whereSql = merged.sql;
  ...
  const loaded = await loadRelations(executor, runtime, table, items, args.with);

  const lastItem = items[items.length - 1];
  const nextCursor = hasMore && lastItem ? cursorFromRow(orderSpec, lastItem) : null;
