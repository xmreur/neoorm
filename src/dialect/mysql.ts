...
export function mysqlColumnType(
	...
	if (
		...
		throw schemaError(
			...
			`PostGIS column kind "${col.kind}" is not supported on MySQL`,
		);
	}
	...
	switch (col.kind) {
		...
		case "citext": {
			...
			return `VARCHAR(${width}) COLLATE utf8mb4_0900_ai_ci`;
		}
		case "enum": {
...
function columnDef(
	...
	const sqlType = mysqlColumnType(col, manifest);
...
function emitCreateTable(
	...
	for (const col of table.columns) {
		lines.push(`  ${columnDef(col, table, manifest, mysqlDialect)}`);
	}
...
function emitCreateIndex(table: ManifestTable, index: ManifestIndex): string {
	if (index.whereSql) {
		compileError("Partial indexes are not supported on MySQL", {
...
function emitDropIndex(indexName: string, tableSqlName?: string): string {
	if (tableSqlName) {
...
function emitAlterColumn(
	...
	if (alter.setType) {
		const typeSql = mysqlColumnType(alter.setType, manifest);
		const notNull =
		...
		const typeSql = existing ? mysqlColumnType(existing, manifest) : "TEXT";
		stmts.push(
			alter.setNullable
	...
	if (alter.setCheckExpression !== undefined) {
		...
		stmts.push(`ALTER TABLE ${tableName} DROP CHECK ${q(constraintName)};`);
...
function emitAlterTable(table: ManifestTable, diff: TableDiff): string[] {
	...
	if (diff.dropIndexes) {
		for (const indexName of diff.dropIndexes) {
			stmts.push(emitDropIndex(indexName, table.sqlName));
		}
	}
	...
	if (diff.renameColumns) {
		for (const { from, to } of diff.renameColumns) {
			...
			const typeSql = nextCol
				? mysqlColumnType(nextCol, manifest)
				: "TEXT";
			stmts.push(
	...
	if (diff.addColumns) {
		for (const col of diff.addColumns) {
			stmts.push(
				`ALTER TABLE ${tableRef(table)} ADD COLUMN ${columnDef(col, table, manifest, mysqlDialect)};`,
			);
		}
	}
