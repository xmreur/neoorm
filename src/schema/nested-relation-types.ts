      ...
      ? TColumns[K]["_meta"]["target"] extends `${infer Sql}.${string}`
        ? SqlNameToAccessor<TSchema, Sql> extends infer Acc extends keyof TSchema & string
          ? Acc extends keyof TSchema & string
            ? ShallowToOneRelationWriteForAccessor<TSchema, Acc, TColumns[K]>
            : never
          : never
        : never
      : never
...
> = {
  ...
  > extends { target: infer TTarget extends string }
    ? FkTargetMatchesAccessor<TSchema, TTarget, TParentAccessor> extends true
      ? K
      : never
    : never;
...
> = Omit<
  Expand<
    InferInsertRow<TSchema[TChildAccessor]["_columns"]> &
      ShallowOutgoingFkRelationWriteMap<TSchema, TSchema[TChildAccessor]["_columns"]>
  >,
...
> = IsThroughTable<TSchema[TSourceAccessor]["_columns"]> extends true
  ? never
  : {
      [K in FkColumnNames<TSchema[TSourceAccessor]["_columns"]> as TSchema[TSourceAccessor]["_columns"][K] extends FkBuilder<
      ...
      >
        ? FkTargetMatchesAccessor<TSchema, TTarget, TAccessor> extends true
          ? Inv
          : never
        : never]?: ToManyRelationWrite<TSchema, TAccessor, TSourceAccessor>;
    };
