...
<p>
  ...
  PostgreSQL, SQLite, MySQL 8, and MariaDB 10.11 dialects ship today.
</p>
...
bun add neoorm mariadb   # MariaDB 10.11
```

Requires **Node.js 22.5+** or **Bun** (SQLite uses built-in `node:sqlite` / `bun:sqlite`; see [SQLite](docs/sqlite.md). MySQL needs `mysql2`; see [MySQL](docs/mysql.md). MariaDB needs the official `mariadb` connector; see [MariaDB](docs/mariadb.md)).
...
- **PostgreSQL powered** — arrays, JSONB, PostGIS, enums, full-text, extensions — no abstraction layer that gets in the way. SQLite ships for local dev and single-file deployments; MySQL 8 ships via optional `mysql2`; MariaDB 10.11 ships via the official `mariadb` connector.
...
| [MariaDB](docs/mariadb.md) | MariaDB 10.11 dialect, official mariadb connector, type mapping, limitations |
...
- **PostgreSQL, SQLite, MySQL, and MariaDB first** — we ship Postgres, SQLite, MySQL 8, and MariaDB 10.11 dialects and lean into each one's strengths.
