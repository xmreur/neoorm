...
import type { Manifest, ManifestColumn, ManifestTable } from "../src/dialect/types.js";
import {
  diffManifest,
  columnsEqual,
  resolveMigrationSql,
...
describe("diffManifest", () => {
  it("generates initial migration with extensions and non-unique indexes", () => {
    const next = manifest({
      users: table("users", "users", [col("id", "id", { kind: "id", primary: true, nullable: false })], {
        indexes: [
          {
            name: "emailIdx",
  ...
  it("detects new tables", () => {
    ...
    const next = manifest({
      ...prev.tables,
      posts: table("posts", "posts", [col("id", "id", { kind: "id", primary: true, nullable: false })]),
    });
  ...
  it("detects add column, drop column, and rename column", () => {
    ...
    const { sql, destructive } = diffManifest(prev, next);
    expect(sql.some((s) => s.includes('RENAME COLUMN "email_address" TO "email"'))).toBe(true);
  ...
  it("detects column type changes as destructive", () => {
    ...
    const { sql, destructive } = diffManifest(prev, next);
    expect(sql.some((s) => s.includes('ALTER COLUMN "age" TYPE'))).toBe(true);
    expect(destructive).toEqual([]);
  });
  ...
  it("detects index add and drop", () => {
    ...
    const { sql, destructive } = diffManifest(prev, next);
    expect(sql.some((s) => s.includes('DROP INDEX IF EXISTS "users_emailIdx_idx"'))).toBe(true);
    expect(sql.some((s) => s.includes('CREATE INDEX "users_nameIdx_idx"'))).toBe(true);
  ...
  it("detects foreign key add, drop, and change", () => {
    ...
    const { sql, destructive } = diffManifest(prev, next);
    expect(sql.some((s) => s.includes('DROP CONSTRAINT "posts_author_id_fkey"'))).toBe(true);
    expect(sql.some((s) => s.includes('DROP COLUMN "author_id"'))).toBe(true);
    expect(sql.some((s) => s.includes('ADD COLUMN "editor_id"'))).toBe(true);
  ...
  it("matches indexes by sqlName when TS extra names differ (db push path)", () => {
    const prev = manifest({
      accounts: table("accounts", "account", [
        col("id", "id", { kind: "id", primary: true, nullable: false }),
        col("userId", "user_id", {}),
      ], {
        indexes: [
  ...
  it("aligns fk column types with referenced primary keys", () => {
    ...
    const next = manifest({
      professors: table("professors", "professor", [
        col("id", "id", { kind: "uuid", primary: true, nullable: false }),
      ]),
      workingHours: table("workingHours", "ProfessorWorkingHour", [
        col("id", "id", { kind: "uuid", primary: true, nullable: false }),
        col("professorId", "professor_id", {
    ...
    expect(
      sql.some((s) =>
        s.includes('ALTER TABLE "ProfessorWorkingHour" ALTER COLUMN "professor_id" TYPE UUID USING "professor_id"::uuid'),
      ),
    ).toBe(true);
  ...
  it("emits USING when changing text id columns to uuid", () => {
    ...
    const { sql } = diffManifest(prev, next);
    expect(sql.some((s) => s.includes('TYPE UUID USING "id"::uuid'))).toBe(true);
  });
