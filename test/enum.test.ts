...
import { defineSchema, table, enumType, text } from "neoorm/schema";
...
import { postgresDialect, emitCreateEnumTypes } from "../src/dialect/postgres.js";
...
const schema = defineSchema({
  posts: table("posts", {
    ...
    status: enumType(["draft", "published"] as const).notNull().default("draft"),
  }),
});
...
describe("enumType column", () => {
  ...
  it("defaults to check mode with inline CHECK constraint", () => {
    ...
    expect(status?.checkExpression).toBe('"status" IN (\'draft\', \'published\')');
  ...
  it("supports union mode without DB check", () => {
    const manifest = schemaToManifest(schema, undefined, undefined, { enumMode: "union" });
    const posts = requirePostsTable(manifest);
    const status = posts.columns.find((c) => c.tsName === "status");
  ...
  it("supports native postgres enum mode", () => {
    const manifest = schemaToManifest(schema, undefined, undefined, { enumMode: "native" });
    const posts = requirePostsTable(manifest);
    const status = posts.columns.find((c) => c.tsName === "status");
    ...
    expect(sql).toContain('"status" posts_status NOT NULL DEFAULT \'draft\'');
  ...
  it("flags enum value changes as manual migrations", () => {
    const prev = schemaToManifest(schema, undefined, undefined, { enumMode: "native" });
    const nextSchema = defineSchema({
      posts: table("posts", {
        id: text().notNull().primary(),
        status: enumType(["draft", "published", "archived"] as const).notNull(),
      }),
    });
    const next = schemaToManifest(nextSchema, undefined, undefined, { enumMode: "native" });
    const diff = diffManifest(prev, next);
    expect(diff.destructive.some((change) => change.kind === "alter_enum_manual")).toBe(true);
  });
});
