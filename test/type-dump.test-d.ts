...
import type { RelationCreateMap, OutgoingFkRelations } from "../src/schema/relation-types.js";
...
type _assertAuthorInCreateMap = CreateMap["author"] extends { connect: { id: string } }
  ? true
  : never;
type _assertTagsInCreateMap = CreateMap["tags"] extends {
  connectOrCreate: { where: { slug?: string }; create: { slug: string; name: string } }[];
}
  ? true
  : never;
