...
import { defineSchema, table, id, text, fk, index, bool } from "neoorm/schema";
import { schemaToManifest, validateManifest } from "../src/codegen/schema-to-manifest.js";
import { diffManifest, collectRedundantMapWarnings } from "../src/codegen/generate.js";
...
describe("column map", () => {
  it("uses custom sql names in manifest", () => {
    ...
    expect(users.columns.find((c) => c.tsName === "legacyCode")?.sqlName).toBe("legacy_user_code");

    const posts = manifest.tables["posts"]!;
    expect(posts.columns.find((c) => c.tsName === "authorId")?.sqlName).toBe("author_ref");
  });
  ...
  it("detects sql name change from adding map in migration diff", () => {
    ...
    expect(sql.some((s) => s.includes('RENAME COLUMN "email_address" TO "email"'))).toBe(
      true,
    );
  });
  ...
  it("warns when map matches default snake_case", () => {
    const redundantSchema = defineSchema({
      users: table("users", {
        id: id.primary(),
        emailVerified: bool().notNull().default(false).map("email_verified"),
      }),
    });
