...
import type { Profile, UserPayload } from "../examples/blog/neoorm/models.js";
...
describe("one-to-one relations", () => {
	...
	it("find loads the inverse one-to-one relation as a single object", async () => {
		...
		await loadRelations(executor, runtime, usersTable, parentRows, {
			profile: true,
		});
		...
		const profileQuery = executor.queries.find((q) =>
			q.sql.includes('FROM "profiles"'),
		);
	...
	it("create emits an inverse one-to-one child create with the FK set to the parent id", async () => {
		...
		const profileInsert = executor.queries.find((q) =>
			q.sql.includes('INSERT INTO "profiles"'),
		);
	...
	it("connect links a single existing child to the parent", async () => {
		...
		const connectQuery = executor.queries.find((q) =>
			q.sql.includes('UPDATE "profiles"'),
		);
