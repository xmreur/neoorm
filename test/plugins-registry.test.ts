...
import { type ColumnMeta, createColumnBuilder } from "../src/schema/column.js";
...
describe("plugin registry", () => {
	...
	it("deduplicates plugin registration by name", () => {
		...
		registerPlugin(testPlugin);
		expect(
			getPluginRegistry().filter((p) => p.name === "test-plugin"),
		).toHaveLength(1);
	});

	it("collects extensions from plugins", () => {
		registerPlugin(testPlugin);
		expect(collectExtensions(getPluginRegistry())).toEqual([
			"citext",
			"test_ext",
		]);
	});
