...
import { defineSchema, table, id, text, fk, primaryKey } from "neoorm/schema";
import { schemaToManifest } from "../src/codegen/schema-to-manifest.js";
import { compileWhere, buildFindByIdQuery } from "../src/runtime/query/compile.js";
...
describe("manifest-driven primary keys", () => {
  ...
  it("resolves mapped PK in manifest", () => {
    ...
    expect(requireScalarPrimaryKey(users)).toEqual({ tsName: "id", sqlName: "user_id" });
  });
  ...
  it("compiles findById where using PK ts name and mapped sql column", () => {
    const { sql } = compileWhere(manifest, users, { id: "user_1" }, postgresDialect);
  ...
  it("uses target PK sql column in inverse-many connect SQL", async () => {
    ...
    const updateSql = executor.queries.find((q) => q.sql.includes("UPDATE"));
