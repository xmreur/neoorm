...
import { defineSchema, table, id, text, fk, index } from "neoorm/schema";
...
import { collectRedundantMapWarnings } from "../src/codegen/generate.js";
...
describe("column naming strategies", () => {
  it("keeps snakeCase as the default SQL naming strategy", () => {
    ...
    expect(users.columns.find((c) => c.tsName === "emailAddress")?.sqlName).toBe(
      "email_address",
    );
  });
  ...
  it("allows a global camelCase default with per-table snakeCase override", () => {
    ...
    expect(
      manifest.tables["camelUsers"]?.columns.find((c) => c.tsName === "emailAddress")?.sqlName,
    ).toBe("emailAddress");
    expect(
      manifest.tables["snakeUsers"]?.columns.find((c) => c.tsName === "emailAddress")?.sqlName,
    ).toBe("email_address");
  });
  ...
  it("still lets .map() override the naming strategy", () => {
    ...
    expect(users.columns.find((c) => c.tsName === "emailAddress")?.sqlName).toBe("email");
    expect(users.columns.find((c) => c.tsName === "authorId")?.sqlName).toBe("author_ref");
  });
  ...
  it("diffs naming strategy changes as column renames", () => {
    ...
    expect(
      sql.some((statement) =>
        statement.includes('RENAME COLUMN "email_address" TO "emailAddress"'),
      ),
    ).toBe(true);
  });
