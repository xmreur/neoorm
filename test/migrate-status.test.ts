...
describe("computeMigrationStatus", () => {
  it("lists pending migrations not yet applied", () => {
    const status = computeMigrationStatus(
      ...
      [{ name: "20250101_init", appliedAt: new Date("2025-01-01T12:00:00Z") }],
    );
  ...
  it("detects applied migrations missing on disk", () => {
    const status = computeMigrationStatus(
      ...
      [
        { name: "20250101_init", appliedAt: new Date("2025-01-01T12:00:00Z") },
        { name: "20241201_old", appliedAt: new Date("2024-12-01T12:00:00Z") },
      ],
    );
...
describe("formatMigrateStatus", () => {
  it("renders applied and pending sections", () => {
    const lines = formatMigrateStatus(
      {
        applied: [
          { name: "20250101_init", appliedAt: new Date("2025-01-01T12:00:00.000Z") },
        ],
    ...
    expect(lines.join("\n")).toContain("Migration status (/project/neoorm/migrations)");
