...
import { defineSchema, table, id, text, uuid } from "neoorm/schema";
...
function mockPool(): Pool & { queries: Array<{ sql: string; params: unknown[] }> } {
  ...
  return { query, queries } as unknown as Pool & { queries: Array<{ sql: string; params: unknown[] }> };
}
...
describe("extension management", () => {
  ...
  it("deduplicates overlapping user and plugin extensions", async () => {
    ...
    const manifest = schemaToManifest(schema);
    const postgisCount = manifest.extensions?.filter((e) => e === "postgis").length;
